...
function(elder_validate_optical_ini root tier ini_file)
    ...
    # TECHNIQUE=1 must sit directly under the section header, matching ENB's
    # own save format; index 1 activates the first declared Elder technique
    # instead of ENB's internal DEFAULT shader.
    string(FIND "${content}" "${expected_section}\nTECHNIQUE=1\n" technique_position)
    if(technique_position EQUAL -1)
        message(FATAL_ERROR "Optical preset must set TECHNIQUE=1 directly under ${expected_section}: ${tier}/${ini_file}")
    endif()
    ...
    foreach(line IN LISTS lines)
        ...
        if(line MATCHES "^TECHNIQUE=")
            if(NOT line STREQUAL "TECHNIQUE=1")
                message(FATAL_ERROR "Optical preset technique must be exactly TECHNIQUE=1: ${tier}/${ini_file}: ${line}")
            endif()
            if(seen_technique)
                message(FATAL_ERROR "Duplicate TECHNIQUE key: ${tier}/${ini_file}")
            endif()
            set(seen_technique TRUE)
            continue()
        endif()
        if(NOT line MATCHES "^(.+)=([^;]+)$")
...
function(elder_validate_non_optical_ini root tier ini_file)
    ...
    string(FIND "${content}" "${expected_section}\nTECHNIQUE=1\n" technique_position)
    if(technique_position EQUAL -1)
        message(FATAL_ERROR "Non-optical preset must set TECHNIQUE=1 directly under ${expected_section}: ${tier}/${ini_file}")
    endif()
    file(STRINGS "${candidate}" lines)
    set(seen_keys "")
    set(seen_technique FALSE)
    foreach(line IN LISTS lines)
        ...
        if(line MATCHES "^TECHNIQUE=")
            if(NOT line STREQUAL "TECHNIQUE=1")
                message(FATAL_ERROR "Non-optical preset technique must be exactly TECHNIQUE=1: ${tier}/${ini_file}: ${line}")
            endif()
            if(seen_technique)
                message(FATAL_ERROR "Duplicate TECHNIQUE key: ${tier}/${ini_file}")
            endif()
